const path = require('path');
const express = require('express');
const hbs = require('hbs');
const request = require('postman-request');
const { env } = require('process');

const port = env.PORT || 3000;

const publicpath =  path.join(__dirname,'../public');

const app = express();

//setting up static file path
app.use(express.static(publicpath));

// setting up partials file path
app.set('view engine','hbs');
app.set('views',path.join(__dirname,'../views'));

hbs.registerPartials(path.join(__dirname,'../views/partial'));
app.get('', (req, res) => {
    res.render('index',{title:'Title Page',name: 'Ram'})
});
app.get('/about', (req, res) => {
    res.render('about',{
        title: 'About Page',
        name:'Ram',
        imgsrc:'data:image/jpeg;base64,'
    });
});
app.get('/help',(req,res) => {
    res.render('help',{
        title: 'Help page',
        name:'Ram'
    })
});
app.get('/weather', (req, res) => {
    if(!req.query.place){
        return res.send({
            err: 'Place must be provided'
        })
    }
    const url = `http://api.weatherstack.com/current?access_key=71cb315d69f5ebd2d3b5e46aae41fef9&query=${req.query.place.toLowerCase()}`;
    request({url: url, json: true},(error, response) => {
        const resp = response.body;
        if(resp.error){
            return res.send({msg:'Sorry place not found'});
        }
        return res.send({msg:`The current temperature is ${resp.current.temperature} C in ${resp.location.name} at ${resp.location.localtime}.But it feels like ${resp.current.feelslike}`});
    });
    // let resp ={}
    // switch(req.query.place.toLowerCase()){
    //     case 'pallavaram':
    //         resp = {name:'pallavaram',temp:34};
    //         break;
    //     case 'tirusulam':
    //         resp = {name:'tirusulam',temp:35};
    //         break;

    //     case 'meenambakkam':
    //         resp = {name:'meenambakkam',temp:35};
    //         break;
    //     default:
    //         resp = {name:req.query.place.toLowerCase(),temp:'unknown'};
    //         break;
    // }
    // res.send(resp);
});

app.get('/help/*', (req, res) => {
    res.render('404',{
        title:'Help Page not found',
        errmsg:'Sorry the help page you are looking for is not present',
        name:'Ram'
    })
});

app.get('*', (req, res) => {
    res.render('404',{
        title:'Page not found',
        errmsg:'Sorry the page you are looking for is not present',
        name:'Ram'
    })
});

app.listen(port, () => {
    console.log('app is running in port 3000');
});